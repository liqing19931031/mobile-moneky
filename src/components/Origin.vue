<template lang="html">
  <div class="origin-group">
    ...
    <div class="top-nav clearfix">
      <router-link :to="{path: '/detail', query: {id: item.id}}" v-for='(item, index) in this.imgs' :key='item.id'>
        <div class="one-nav" :style='{background: `url(/static/origin/0-${index}.png)`, backgroundSize: "100% 100%"}' v-if='index !== 0'>
      ...
      <div class="zone" v-for='(item, index) in this.shops' :id='index'>
        ...
        <div class="zone-content clearfix">
          <div class="zone-one" v-for='(shop, childindex) in item.shopData' v-if='index !== 2'>
            <router-link :to="{path: '/detail', query: {id: shop.id}}">
          ...
          <div class="zone-one--shop" v-else>
            <a :href='`${BASEURL}store.html?shop_id=${shop.id}`'>
              <img class="lazyImg in" src='/static/load.jpg' alt="" :data-url='`/static/origin/${index + 2}-${childindex + 1}.png`'>
              <div class="des">
...
export default {
  data () {
    return {
      ...
      imgs: [
        '',
        {name: '源于江南 追求生活', id: 258},
        {name: '经典职业装 量大从优', id: 270},
        {name: '休闲T恤 舒适有型', id: 410},
        {name: '新款连衣裙 穿出自己', id: 503},
        {name: '经典职业装 量大从优', id: 339},
        {name: '厂家直销 各款式定做', id: 2438},
        {name: '雨刷精 防冻更给力', id: 2480},
        {name: '四十周年 限量纪念版', id: 2475},
        {name: '个性印花 打造自我', id: 437}
      ],
      shops: [
        {
          name: '优品好货',
